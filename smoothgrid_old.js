...
const showColumnaRegistro = (tr, pagina, indexGeneral, indexPagina)=>{
  console.log("================================================")
  console.log("AQUI HAY QUE TRABAJAR LA ESCTRUTURA PARA DATOS CON FUNCIONES")
  //console.log(tr)
  //console.log("Pagina", pagina)
  //console.log("index general", indexGeneral)
  //console.log("index en pagina", indexPagina)
 //console.log("Datos Paginados:", datosPaginados)
...
if(imagenDesplegar.classList.contains('smoothgrid_opened')){
  ...
  //? Agregar la nueva TR vertical con los datos que fueroon suprimidos

  //=================== Buscando los datos que fueron suprimidos start =================
        //console.log("Columnas visualizadas:", cantidadColumnasVisualizadas)
        const datosPaginaActual = datosPaginados[pagina-1];
        const datosRegistroClickeado = datosPaginaActual.datos[indexPagina-1];
        //console.log(datosRegistroClickeado)
        ...
        for (let i = cantidadColumnasVisualizadas-1; i < clavesRegistroClikeado.length; i++) {
          const categoria = clavesRegistroClikeado[i];
            elementosListas += `<li><span><label class="smooth-category">${datosHeader[i]}:</label> ${datosRegistroClickeado[categoria]}</span></li>`;
          
        }

  //=================== Buscando los datos que fueron suprimidos end ====================
  
  const nuevaTR = document.createElement("tr")
  nuevaTR.classList.add("child_tr_smoothgrid");
  nuevaTR.innerHTML=
  ` <td class="full-details-smootgrid" colSpan="${cantidadColumnasVisualizadas}" >
    <ul class="smoothgrid-ul-collapsed" >
    ${elementosListas}
    </ul>
  </td>`;
...
const componentePaginacion = (paginaActiva, cantidadPaginas) => {
  ...
  const handlePaginaCambio = (numero) => {
    ...
    mensajeCantidadDatosMostrados (datosPaginados[numero - 1], datosBody);
...
const renderizarTabla= (mostrarDatosHeader, mostrarDatosBody) =>{
  let headerTableString = mostrarDatosHeader.map(element =>{
    if (element) {
        return  `<th>${element}</th>`;
    }
    return ''; // Ignorar elementos undefined o cadenas vacías
  }).join('')
  ...
  if (mostrarDatosBody.length > 0) {
    ...
    bodyTableString = mostrarDatosBody.map(objeto => {
      const clavesObjeto = Object.keys(objeto);
      let TDs = "";
      
      clavesObjeto.forEach(element => {
        const td = `<td>${objeto[element]}</td>`;
        TDs += td;
      });
    ...
    bodyTableString = `<tr>
      <td class="full" colSpan="${mostrarDatosHeader.length + 1}" >
    ...
    if(pageComp){
      ...
      const tabla= `
              <table id="smoothTable_${idContendor}">
              <thead>
              <tr>
                    ${headerTableString}
              </tr>
      ...
      contenedorPadre.querySelector(`.cont-table-space-smoothgrid`).innerHTML=tabla;
...
const filtrarPorInput = (event)=>{
  ...
  if (valorEntrada.length > 0) {
      ...
      //? =================================
          ...
          mensajeCantidadDatosMostrados (datosPaginados[paginaActual -1], datosRecopilados);
          ...
          console.log(datosPaginados.length)
